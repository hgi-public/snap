...
static void usage(const char *prog)
{
	printf("Usage: %s [-h] [-v, --verbose] [-V, --version]\n"
	       ...
	       "  -R, --action-repeat <N>   Repeat action N times\n"
	       "\n"
	       "Example:\n"
           "  snap_nvme_cached -A HOST_DRAM -D HOST_DRAM -i in.bin -o out.bin ...\n"
           "  snap_nvme_cached -A HOST_DRAM -D CARD_DRAM -i in.bin -d 0xD000 ...\n"
           "  snap_nvme_cached -A HOST_DRAM -D NVME_SSD  -i in.bin -d 0xE000 ...\n"
           "\n"
           "  snap_nvme_cached -A CARD_DRAM -D HOST_DRAM -a 0xD000 -o out.bin -s 0x200 ...\n"
           "  snap_nvme_cached -A CARD_DRAM -D NVME_SSD  -a 0xD000 -d 0xE000 -s 0x200 ...\n"
           "  snap_nvme_cached -A CARD_DRAM -D CARD_DRAM -a 0xD000 -d 0xD200 -s 0x200 ...\n"
           "\n"
           "  snap_nvme_cached -A NVME_SSD -D CARD_DRAM -a 0xE000 -d 0xD000 -s 0x200 ...\n"
           "  snap_nvme_cached -A NVME_SSD -D HOST_DRAM -a 0xE000 -o out.bin -s 0x200 ...\n"
           "\n"
           ...
           "    but can be done by calling snap_nvme_cached twice.\n"
	       "\n",
	       prog);
}

static void snap_prepare_nvme_cached(struct snap_job *cjob,
				 struct nvme_cached_job *mjob,
				 void *addr_in,
				 uint32_t size_in,
				 uint8_t type_in,
				 void *addr_out,
                 ...
                 uint64_t drive_id)
{
	fprintf(stderr, "  prepare nvme_cached job of %ld bytes size\n", sizeof(*mjob));
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		static struct option long_options[] = {
			...
			{ "action-repeat", required_argument, NULL, 'R' },
		...
		ch = getopt_long(argc, argv,
				 "A:C:i:o:a:S:D:d:n:x:s:t:R:XVqvhI",
	...
	snap_prepare_nvme_cached(&cjob, &mjob,
	...
	fprintf(stdout, "nvme_cached took %lld usec\n",
